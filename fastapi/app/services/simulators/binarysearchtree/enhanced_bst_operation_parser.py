...
from app.services.simulators.binarysearchtree.bst_statement_parser import BSTStatementParser
from app.services.simulators.binarysearchtree.bst_node_manager import BSTNodeManager


class EnhancedBSTOperationParser(OperationParser):
    """Enhanced BST-specific operation parser with better step tracking"""
    
    def __init__(self, context: Dict[str, Any]):
        super().__init__(context)
        self.node_manager = BSTNodeManager(context)
        self.statement_parser = BSTStatementParser(context, self.print_handler)
    
    def parse_and_execute(self, line: str, line_number: int, step_number: int, 
                         steps: List[ExecutionStepSchema], 
                         create_step_func) -> bool:
        """Parse and execute BST operations with enhanced step tracking"""
