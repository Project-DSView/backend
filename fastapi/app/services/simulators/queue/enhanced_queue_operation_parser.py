...
from app.services.simulators.queue.queue_statement_parser import QueueStatementParser
from app.services.simulators.queue.queue_node_manager import QueueNodeManager


class EnhancedQueueOperationParser(OperationParser):
    """Enhanced Queue-specific operation parser with better step tracking"""
    
    def __init__(self, context: Dict[str, Any]):
        super().__init__(context)
        self.node_manager = QueueNodeManager(context)
        # Pass context to statement parser so it can use updated class info
        self.statement_parser = QueueStatementParser(context, self.print_handler)
    
    def parse_and_execute(self, line: str, line_number: int, step_number: int, 
                         steps: List[ExecutionStepSchema], 
                         create_step_func) -> bool:
        """Parse and execute queue operations with enhanced step tracking"""
